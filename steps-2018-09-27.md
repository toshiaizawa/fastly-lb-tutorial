...
* マイクロサービス B URL: `https://<Service_B_name>.appspot.com/`
...
### Step (当日追加): Override host の設定

1. 画面左のメニューから **Settings** をクリック
2. **Override host** を On にして、**Override host header** フィールドに `<Service_B_name>.appspot.com` を記入

### Step 6: オリジンに対するリクエストの URL パス部を変更 (/product/* → /*) → ハンズオンではこのステップを省略しました
...
### Step 4: フェイルオーバー機能の明示的な実装 → ハンズオンではこのステップを省略しました
...
## チュートリアル3 (デモ：Dynamic Servers を用いたロードバランサー) → ハンズオンではこのチュートリアルは実施しませんでした
